...
public class Normalization_inv {
    ...
    static void Task3Main(Path inputPath, Path outputPath) throws Exception {
        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, "2019st13 Normalization_inv Job");
        job.setMapperClass(Normalization_inv.NormMapper.class);
        job.setReducerClass(Normalization_inv.NormReducer.class);
        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(Text.class);
        job.setJarByClass(Normalization_inv.class);
